...
namespace SpiderPositionFix.Patches
{
    ...
    public class SpiderPositionPatch
    {
        ...
        static void UpdatePostfix(SandSpiderAI __instance)
        {
            ...
            if (!__instance.lookingForWallPosition && !__instance.gotWallPositionInLOS && !instanceData.isInWallState)
            {
                if (Vector3.Distance(__instance.meshContainerPosition, __instance.transform.position) > 0.4f && !__instance.onWall && !__instance.agent.isOnOffMeshLink)
                {
                    ...
                    if (debug) InitialScript.Logger.LogDebug("Spider: Applying slowdown. New speed: " + __instance.agent.speed);
                }
                else if (instanceData.applySpeedSlowdown && !__instance.agent.isOnOffMeshLink)
                {
                    ...
                    if (debug) InitialScript.Logger.LogDebug("Spider: Returning original speed");
                }
                if (__instance.agent.isOnOffMeshLink)
                {
                    instanceData.applySpeedSlowdown = true;
                    __instance.agent.speed = instanceData.originalSpeed / 1.25f;
                    if (debug) InitialScript.Logger.LogDebug("Spider: On offMeshLink. Cutting speed");
                }
            }
            else if (instanceData.applySpeedSlowdown && instanceData.isInWallState && !__instance.reachedWallPosition)
            {
                ...
                instanceData.offsetSpeed = 0;
                __instance.agent.speed = instanceData.originalSpeed;
                if (debug) InitialScript.Logger.LogDebug("Spider/2/: Returning original speed");
            }
        }
        ...
        static void MeshContainerPositionFix(SandSpiderAI __instance)
        {
            ...
            {
                if (debug) InitialScript.Logger.LogInfo("Spider: Warping agent to nearest navMesh position to climbPoint: " + spiderData[__instance].climpPoint + ". Distance: " + Vector3.Distance(__instance.agent.transform.position, spiderData[__instance].climpPoint));
            ...
            if (!__instance.lookingForWallPosition && !__instance.gotWallPositionInLOS && !instanceData.isInWallState)
            {
                if (Vector3.Distance(__instance.meshContainerPosition, __instance.transform.position) > 0.75f && !__instance.onWall)
                {
                    ...
                    if (debug) InitialScript.Logger.LogDebug("Spider: distance: " + Vector3.Distance(__instance.meshContainerPosition, __instance.transform.position));
            ...
            if (__instance.onWall && spiderData[__instance].climpPoint == Vector3.zero)
            {
                ...
                if (debug) InitialScript.Logger.LogDebug("Spider: Set new climpPoint: " + spiderData[__instance].climpPoint);
            }
            if (__instance.lookingForWallPosition && __instance.gotWallPositionInLOS && !instanceData.isInWallState || __instance.onWall)
            {
                ...
                if (debug) InitialScript.Logger.LogDebug("Spider: wallState2: " + instanceData.isInWallState);
            }
            if (!__instance.lookingForWallPosition && instanceData.isInWallState && __instance.movingTowardsTargetPlayer)
            {
                ...
                {
                    if (debug) InitialScript.Logger.LogDebug("Spider: returning from wall state and zeroing climp point: " + instanceData.climpPoint + ". Distance: " + Vector3.Distance(__instance.meshContainerPosition, __instance.transform.position));
